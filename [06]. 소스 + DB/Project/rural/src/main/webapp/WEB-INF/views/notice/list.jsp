        ...
        <div class="section-padding2">
            <div class="container">
            	...
            	<div id="menubox">
					...
					<ul class="nav nav-tabs" id="navibar" style="font-size:1em; font-weight: bold; margin-bottom: 20px;" >
						...
						<c:if test="${ category != '전체보기' }">
	               		<li class="nav-item">
	               			<a class="nav-link disabled" href="/rural/notice/list.do?category=전체보기&sort=${ sort }&column=${ colum }&search=${ search }#board">
	               			전체보기</a>
	               		</li>
						</c:if>
						...
						<c:if test="${ category == '전체보기' }">
	               		<li class="nav-item">
	               			<a class="nav-link active" href="/rural/notice/list.do?category=전체보기&sort=${ sort }&column=${ colum }&search=${ search }#board">
	               			전체보기</a>
	               		</li>
						</c:if>

						<c:if test="${ category != '지역행사' }">
						<li class="nav-item">
							<a class="nav-link disabled" href="/rural/notice/list.do?category=지역행사&sort=${ sort }&column=${ colum }&search=${ search }#board">
							지역행사</a>
						</li>
						</c:if>
						...
						<c:if test="${ category == '지역행사' }">
						<li class="nav-item">
							<a class="nav-link active" href="/rural/notice/list.do?category=지역행사&sort=${ sort }&column=${ colum }&search=${ search }#board">
							지역행사</a>
						</li>
						</c:if>
						
						<c:if test="${ category != '이벤트' }">
						<li class="nav-item">
							<a class="nav-link disabled" href="/rural/notice/list.do?category=이벤트&sort=${ sort }&column=${ colum }&search=${ search }#board">
							이벤트</a>
						</li>
						</c:if>
						<c:if test="${ category == '이벤트' }">
						<li class="nav-item">
							<a class="nav-link active" href="/rural/notice/list.do?category=이벤트&sort=${ sort }&column=${ colum }&search=${ search }#board">
							이벤트</a>
						</li>
						</c:if>
						...
						<li class="nav-item">
							<a class="nav-link disabled" href="/rural/notice/list.do?category=공지사항&sort=${ sort }&column=${ colum }&search=${ search }#board">
							공지사항</a>
						</li>
						</c:if>
						...
						<c:if test="${ category == '공지사항' }">
						<li class="nav-item">
							<a class="nav-link active" href="/rural/notice/list.do?category=공지사항&sort=${ sort }&column=${ colum }&search=${ search }#board">
							공지사항</a>
						</li>
						</c:if>
    		    ...
    		    <!-- 게시글 리스트 정렬순 ( 최신순 / 댓글순 / 조회수 ) -->
         		<div style="margin-top: 10px; margin-bottom: 5px; float: right; font-size:14px;" id="board">
         			<a href="/rural/notice/list.do?sort=최신순&category=${ category }&column=${ colum }&search=${ search }">최신순</a>&nbsp;/&nbsp;
         			<a href="/rural/notice/list.do?sort=조회순&category=${ category }&column=${ colum }&search=${ search }">조회순</a>
           		</div>
           		...
           		<table class="table">
					...
					<tr id="list">
						...
						<td>
							<!-- 제목 클릭시 이동 -->
							<a href="/rural/notice/view.do?seq=${ dto.seq }&column=${ map.column }&search=${ map.search }" id="content">${ dto.title }</a>
           		...
           		<div class="row justify-content-md-center" style="width: 100%; margin-top: 30px;">
	            	<form method="GET" action="/rural/notice/list.do" style="display:inline-block;">
           		...
           		<c:if test="${ not empty id && lv == 3 }">
					<input type="button" class="btn btn-success" value="글쓰기" style="float: right;"
						onclick="location.href='/rural/notice/add.do?${ seq }';">
				</c:if>
