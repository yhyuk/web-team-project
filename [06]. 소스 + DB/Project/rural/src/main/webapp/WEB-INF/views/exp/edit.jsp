...
<body>
	...
	<div class="favourite-place section-padding2">
		...
		<form method="POST" action="/rural/exp/editok.do" enctype="multipart/form-data">
			<div class="container registAdd">
				...
				<div class="row justify-content-start">
					...
					<div class="col-sm-3">
						<select class="custom-select" name="expInfo" id="expInfo" required>
				...
				<div class="row justify-content-start">
					...
					<div class="col-sm-8">
						<input type="text" class="form-control col-sm-6"
							placeholder="마을이름을 입력해주세요." name="town" value="${dto.town}" required>
					</div>
				</div>
				...
				<div class="row justify-content-start">
					...
					<div class="col-sm-9">
						<input type="date" class="col-sm-3 form-control" name="startDate" id="startDate" required> <input type="date"
							class="col-sm-3 form-control" name="endDate" id="endDate" required>
					</div>
				</div>
				...
				<div class="row justify-content-start">
					...
					<div class="col-sm-8">
						<input type="file" name="image" class="col-sm-6" accept=".gif, .jpg, .png">
						<span>${dto.image}</span>
						<input type="hidden" name="origin" value="${dto.image}">
					</div>
				</div>
   ...
   // 대분류의 value 값이 받은 대분류명일때 selected 프로퍼티를 준다.
 		$("#expInfo").val('${dto.expInfo}').prop("selected",true);
 		$('input[name=name]').val('${dto.name}');
 		$('input[name=totalPerson]').val('${dto.totalPerson}');
 		$('input[name=startDate]').val('${dto.startDate}');
 		$('input[name=endDate]').val('${dto.endDate}');
 		$('input[name=tel]').val('${dto.tel}');
 		$('input[name=address]').val('${dto.address}');
 		$('input[name=site]').val('${dto.site}');
 		
 		
 		$('textarea[name=townDetail]').val('${dto.townDetail}');
 		$('textarea[name=detail]').val('${dto.detail}');
